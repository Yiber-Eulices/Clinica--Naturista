                                            ...
                                            $(document).ready(function(){
                                                $('#CalendarioWeb').fullCalendar({
                                                    header:{
                                                        ...
                                                        <% for (Cita cit: cita) { 
                                                            if (cit.getIddoctor().getIdpersona()== (Integer) session.getAttribute("UsuarioId") ){
                                                        %>
                                                            {
                                                                title  : 'Cita de : <%=cit.getIdespecialidad().getNombre()%>',
                                                                Observaciones : "Observaciones : <%=cit.getObservaciones() %>",
                                                                Paciente : "Paciente : <%=cit.getIdpaciente().getNombre()%><%=cit.getIdpaciente().getApellido()%>",
                                                                Genero : "Genero : <%=cit.getIdpaciente().getGenero()%>",
                                                                Telefono : "Telefono : <%=cit.getIdpaciente().getTelefono() %>",
                                                                Correo : "Correo : <%=cit.getIdpaciente().getCorreo()%>",                                                                
                                                                Rh : "Rh : <%=cit.getIdpaciente().getRh()%>",
                                                        ...
                                                        <% } } %> 
                                                                ...
                                                                $('#Genero').html(calEvent.Genero);
                                                                $('#Telefono').html(calEvent.Telefono);
                                                                $('#Correo').html(calEvent.Correo);
                                                                $('#Rh').html(calEvent.Rh);
